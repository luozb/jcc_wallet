 ...
 */
const buildJCWallet = (password, wallet, walletname, callback) => {
    let secret = ''
    let address = ''
    let count = 0
    let walletID
    ...
    while (!isValidAddress(address) || !isValidSecret(secret)) {
        walletID = sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(walletname.toLowerCase()))
        let keypairs = wallet
        keypairs.type = 'swt'
 ...
 */
const getJCWallet = (walletname) => {
    let walletID = sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(walletname.toLowerCase()))
 ...
 * @param {function} callback
 */
const setJCWallet = (jcWallet, walletname, callback) => {
    let walletID = sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(walletname.toLowerCase()));
 ...
 */
const clearJCWallet = (walletname) => {
    let walletID = sjcl.codec.hex.fromBits(sjcl.hash.sha256.hash(walletname.toLowerCase()))
    Lockr.set(walletID, {})
}
